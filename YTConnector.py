...
getting credentials (OAuth), and acquiring the proper data. It will access local, private files.
...
*get_playlists(doVerify = False): returns a dictionary of playlists for your account with format title:id. Verifies to see correct account if needed
*get_playlist_items(playlist_name = None, playlistId = None, doVerify = False): gets a list of playlist items (pages) having specified a playlist by name or id. These pages
 are parsed apart into seperate song entries and returned.
...
if __name__ == "__main__":
	...
	target = 'test_list' #input("What playlist would you like to access? Capitalization matters.\n")


	songs = get_playlist_items(playlist_name = target)
	for song in songs:
		print(song['snippet']['resourceId']['videoId'])
